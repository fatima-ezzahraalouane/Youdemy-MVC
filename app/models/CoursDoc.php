...
class CoursDoc extends Cours
{
    public function ajouterCours()
    {
        try {
            $stmt = $this->pdo->prepare(
                ...
                VALUES (:titre, :description, :image_url, :contenu, 'Document', :statut, :id_usersite, :id_categorie)"
            );
            ...
            return "Cours document ajouté avec succès.";
        } catch (PDOException $e) {
            throw new Exception("Erreur lors de l'ajout du cours document : " . $e->getMessage());
        }
    }
    ...
    public function afficherCours($page = 1, $limit = 9)
    {
        try {
            ...
            $stmt = $this->pdo->prepare(
                "SELECT * FROM cours WHERE contenu_type = 'Document' AND statut = 'publie' LIMIT :limit OFFSET :offset"
            );
            ...
            throw new Exception("Erreur lors de l'affichage des cours document : " . $e->getMessage());
        }
    }
    ...
    public function rechercherCoursParTitre($titre, $page = 1, $limit = 9)
    {
        try {
            $offset = ($page - 1) * $limit;
            $query = "SELECT * FROM cours WHERE contenu_type = 'Document' AND titre ILIKE :titre AND statut = 'publie' LIMIT :limit OFFSET :offset";
            ...
            throw new Exception("Erreur lors de la recherche des cours document : " . $e->getMessage());
        }
    }
    ...
    {
        try {
            $query = "SELECT COUNT(*) as total FROM cours WHERE contenu_type = 'Document' AND titre ILIKE :titre";
            ...
            throw new Exception("Erreur lors de la récupération du total des cours document : " . $e->getMessage());
        }
    }
